...
<head>
    <title>BriefNow | Newsletter</title>
    <meta charset="UTF-8">
        ...
        <div class="sidebar bg-gradient-primary">
            ...
            <div class="sidebar-wrapper">
                <ul class="nav">
                    ...
                    <hr class="sidebar-divider">
                    <li class="active"><a href="newsletter.php"><i class="fas fa-envelope-open"></i>
                            <p>Newsletter</p>
                        </a></li>
                </ul>
            </div>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title card-heading">Newsletter subscription</h5>
                            </div>
                            <div class="card-body ">
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            while ($row = $result->fetch_array()) { ?>
                                                <tr>
                                                    <th scope="row"><?php echo $row['subscription_id']; ?></th>
                                                    <td><?php echo $row['subscription_name']; ?></td>
                                                    <td><?php echo wordwrap($row['subscription_email'], 25, "<br>\n", TRUE); ?></td>
                                                    <td><?php if ($row['subscription'] == 1) {
                                                            echo "Yes";
                                                        } else {
                                                            echo "No";
                                                        } ?>
                                                    </td>
                                                    <td><?php $date = $row['subscription_date'];
                                                        $activationdate = date('d M Y', strtotime($date));
                                                        echo "$activationdate"; ?></td>
                                                    <td class="text-center">
                                                        <form action="" method="post" id="delete-newsletter-form">
                                                            <input type="hidden" name="newsletterid" class="newsletterid">
                                                        </form>
                                                        <a class="text-danger btn btn-sm btn-link delete-newsletter-btn" newsletterid="<?php echo $row['subscription_id']; ?>">
    ...
    <script>
        ...
        $(document).ready(function() {
            $(document).on('click', '.delete-newsletter-btn', function() {
                var tr = this;
                var newsletterid = $(this).attr('newsletterid');
                console.log(newsletterid);
                $(".newsletterid").val(newsletterid);
                var formData = $('#delete-newsletter-form').serialize();
                console.log(formData);
                $.ajax({
                    url: 'query.php',
                    method: 'post',
                    data: formData + '&action=deletenewsletter'
